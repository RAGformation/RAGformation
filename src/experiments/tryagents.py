...
from prompts import (
    txt_2_diagram_prompt_template,
    fix_import_prompt_template,
    fix_and_write_code_template,
)
import subprocess
...
class ConciergeWorkflow(Workflow):
    ...
    ) -> ConciergeEvent | OrchestratorEvent | StopEvent:
        system_prompt = """
    ...
    async def orchestrator(
        ...
        if response.tool_calls:
            for tool_call in response.tool_calls:
                print(
                    f"Tool {tool_call.tool_name} called with args: {tool_call.tool_kwargs}"
                )
                tool = next(
                    (t for t in self.tools if t.metadata.name == tool_call.tool_name),
                    None,
                )
                if tool:
