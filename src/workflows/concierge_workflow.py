...
from raw_tool_fuctions.diagram_tools import (
    _run_and_check_syntax,
    _suggest_imports,
    _fix_and_write_code,
    _generate_diagram,
)
...
class ConciergeWorkflow(Workflow):
    ...
    async def concierge(
        ...
        concierge = ctx.data["concierge_agent"]
    ...
    async def orchestrator(
    ...
    ) -> ConciergeEvent | TextToDiagramEvent | TextToRAGEvent | PriceLookupEvent| StopEvent:
        ...
        - **Price Lookup**: For looking up price for specific resource.
        ...
        response = str(ctx.data["orchestrator_agent"].chat(ev.request))
    ...
    ) -> OrchestratorEvent | ConciergeEvent:
    ...
    async def text_to_rag(
        ...
        def search_rag(text: str) -> str:
            ...
            self.log_history(ctx, "rag_response", response)
    ...
    @step(pass_context=True)
    async def price_calculation_step(
        self, ctx: Context, ev: PriceLookupEvent
    ) -> ConciergeEvent:
        print(f"Price lookup event request: {ev.request}")
        self.log_history(ctx, "price_lookup", "user", ev.request)

        def get_price(resource_name: str) -> str:
            """
            Use this tool to perform a price lookup for AWS service.
            Input: resource_name representing the user's AWS resource.
            """
            print(f"Performing a price_lookup from text: {resource_name}")
            response = get_price_for_service(resource_name)
            response = f"### User query: \n {text}\n ### Results from price lookup:\n {response}"
            print(response)

            ctx.data["price_lookup_response"] = response

            print(f'price lookup done {ctx.data["price_lookup_response"]}')
            return response
        ...
        if "price_lookup_agent" not in ctx.data:
            ...
            system_prompt = f"""
                ...
                Instructions:
                1. Analyze the user's current architecture provided.
                2. Identify and extract the resources used in the architecture, use us-east-1 as default region unless specified.
                3. Map each resource to its corresponding AWS service name.
                4. Use the the *get_price* and get pricing for all resources.
                5. Assume all default values and provide the pricing considering the assumption.
                """
